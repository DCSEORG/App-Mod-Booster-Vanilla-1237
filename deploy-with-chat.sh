...
# =============================================================================
# deploy-with-chat.sh - Deploy Expense Management App with GenAI (Azure OpenAI + AI Search)
# =============================================================================
...
#   4. Run: bash deploy-with-chat.sh
#
# Note: GenAI resources will be deployed in swedencentral (OpenAI) and uksouth (AI Search)
#       The chat UI gives dummy responses if GenAI is not deployed.
# =============================================================================
...
DEPLOYMENT_OUTPUT=$(az deployment group create \
  ...
  --parameters location=$LOCATION adminObjectId=$ADMIN_OBJECT_ID adminLogin=$ADMIN_LOGIN deployGenAI=true \
...
OPENAI_MODEL=$(echo $DEPLOYMENT_OUTPUT | jq -r '.openAIModelName.value')
...
az webapp config appsettings set \
  ...
  --settings \
    ...
    "OpenAI__DeploymentName=${OPENAI_MODEL}" \
  --output none
...
# Include chatui in the published output
mkdir -p publish/wwwroot/chatui
cp -r chatui/* publish/wwwroot/chatui/

cd publish
zip -r ../app.zip . -q
cd ..
echo "  ✓ Application packaged (with Chat UI)"
...
echo "  Chat UI:      https://${APP_SERVICE_NAME}.azurewebsites.net/chatui/index.html"
...
echo "  NOTE: If GenAI not deployed, chat returns a helpful dummy response"
echo ""
